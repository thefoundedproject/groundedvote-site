...
async function onCheckoutCompleted(session) {
  ...
  const entity = md.entity ?? 'Rooted Reclaimers LLC'
...
// API version 2025-03-31+ (including this account's 2026-05-27.dahlia) moved
// current_period_start/end OFF the Subscription object and onto each subscription
// item. Read the item first, fall back to the older top-level fields, then to
// created/now — so the required Prisma DateTime columns always get a valid value.
function subPeriod(sub) {
  const item = sub.items?.data?.[0]
  const start = item?.current_period_start ?? sub.current_period_start ?? sub.created ?? Math.floor(Date.now() / 1000)
  const end = item?.current_period_end ?? sub.current_period_end ?? start
