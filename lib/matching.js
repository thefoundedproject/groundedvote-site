...
export async function computeAlignment(sessionId, importanceMap = {}) {
  const session = await prisma.quizSession.findUnique({
    ...
    include: {
      ...
      race: {
        // Only candidates still in the race can appear in matches —
        // primary losers and withdrawals keep their data but stop scoring
        include: { candidates: { where: { status: { notIn: ['LOST_PRIMARY', 'WITHDREW'] } } } },
      },
    },
  })
