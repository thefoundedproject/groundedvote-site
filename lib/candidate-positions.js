...
import { fetchCampaignPlatform } from './campaign-site.js'
...
// State-legislature races have thin evidence (no VoteSmart/Congress.gov),
// so scoring is mostly party- and platform-inference — Haiku handles that
// at a fifth of Sonnet's price. Federal races keep Sonnet.
const STATE_POSITION_MODEL = process.env.STATE_POSITION_MODEL || 'claude-haiku-4-5-20251001'
...
async function buildCandidateContext(candidate) {
  const office = candidate.race?.level === 'STATE'
    ? (candidate.race?.label ?? 'State legislature').replace(/ \d{4}$/, '')
    : candidate.race?.chamber === 'SENATE' ? 'U.S. Senate' : `U.S. House, District ${candidate.race?.district}`
  const lines = [
  ...
  // âââ 2c. Campaign website platform (the VoteSmart replacement) ââââââââââ
  // URL comes from a reviewed source (state filing list), stored on the
  // candidate row - never discovered automatically.
  if (candidate.website) {
    const platform = await fetchCampaignPlatform(candidate.website)
    if (platform) { lines.push(platform); lines.push('') }
  }
...
async function scorePositionForQuestion(candidateContext, question, model = POSITION_MODEL) {
...
3. Assign a confidence score from 0.0 to 1.0:
   ...
   0.60â0.78 = Candidate's own campaign-website platform statements (self-report,
               unverified - score what they SAY; confidence reflects that it is said, not done)
...
CRITICAL RULES:
- Evidence that does NOT address this question is ABSENCE of evidence, not neutrality.
  In that case fall back to party-informed inference: score the way the candidate's party
  typically leans on this issue, evidenceType PARTY_INFERENCE, confidence 0.40-0.55.
- Use 3 (neutral) only for genuinely mixed/centrist records, or when even party gives no signal.
- Label evidenceType CAMPAIGN_PLATFORM only when the campaign-website text above actually
  speaks to this question's subject. A site that is silent on the topic is not platform evidence.
...
export async function extractPositionsForRace(raceId, { overwrite = false } = {}) {
  ...
  for (const candidate of candidatesWithRace) {
    ...
    for (const question of race.questions) {
      ...
      try {
        const result = await scorePositionForQuestion(
          context, question,
          race.level === 'STATE' ? STATE_POSITION_MODEL : POSITION_MODEL
        )
        ...
        summary.failed++
        // A billing/credit failure dooms every remaining call — stop the
        // whole race now instead of burning through thousands of 400s.
        // The worker halts its sweep and resumes after credits return.
        if (isBillingError(err)) {
          const billing = new Error(`Anthropic credit balance exhausted (while scoring ${candidate.lastName})`)
          billing.isBillingError = true
          throw billing
        }
      }
    }
...
/** Anthropic 400 "credit balance is too low" — a hard stop, never transient. */
