...
/**
 * Score only incumbents that have no score yet. Idempotent and cheap:
 * candidates without evidence-backed positions return early before any
 * model call, so this is safe to run after every enrichment pass.
 */
export async function scoreUnscoredIncumbents() {
  const incumbents = await prisma.candidate.findMany({
    where: { bioguideId: { not: null }, rhetoricConsistencyScore: null },
  ...
  for (const c of incumbents) {
    try {
      ...
      if (r) {
        summary.scored++
        console.log(`[rhetoric] ${c.firstName} ${c.lastName}: ${r.score}%`)
      } else {
        summary.skipped++
      }
    } catch (err) {
