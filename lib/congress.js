...
/**
 * Get a member's cosponsored legislation.
 * NOTE: the Congress.gov API has no per-member votes endpoint —
 * `/member/{id}/votes` returns 404 "Unknown resource". Cosponsored bills
 * are the closest per-member behavioral record the API exposes, so this
 * replaces the old (never-working) votes fetch. Keep the export name so
 * existing callers pick up real data without changes.
 */
export async function getMemberVotes(bioguideId, limit = 50) {
  const data = await congressFetch(`/member/${bioguideId}/cosponsored-legislation`, { limit })
