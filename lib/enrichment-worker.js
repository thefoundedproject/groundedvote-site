...
export async function runEnrichmentWorker() {
  ...
  try {
    ...
    for (const race of races) {
      ...
      try {
        ...
        if (err.isBillingError) {
          console.error('[enrich] HALTING SWEEP — Anthropic credits exhausted. Top up at console.anthropic.com; the worker resumes on next boot.')
          state.stopRequested = true
          break
        }
      }
    ...
    // Follow-up pass: score record-vs-rhetoric for any incumbent still
    // unscored. Runs after every completed sweep (including no-op boots)
    // and is idempotent — incumbents without evidence return early.
    if (!state.stopRequested) {
      try {
        const { scoreUnscoredIncumbents } = await import('./rhetoric.js')
        const r = await scoreUnscoredIncumbents()
        console.log(`[enrich] rhetoric follow-up — scored=${r.scored} skipped=${r.skipped} failed=${r.failed}`)
      } catch (err) {
        console.error('[enrich] rhetoric follow-up failed:', err.message)
      }
    }
