...
// Clients are created on first use — constructing them at module scope
// crashes `next build` (page-data collection) when API keys are absent.
...
// Env-overridable so state-race question runs can use a cheaper model
// (GENERATION_MODEL=claude-sonnet-4-6) without changing federal behavior.
...
export async function generateVariants(position, topic) {
...
export async function scoreVariants(variants) {
...
Return only the JSON object, no other text.`
  ...
  return withTimeoutAndRetry(async (signal) => {
    ...
    // json_object mode forces an object wrapper and models vary in how
    // they wrap — normalize every shape we have seen to a plain array.
    let scores = null
    ...
    // Recompute totals defensively — selection sorts on them
...
export async function selectBestVariant(variants, scores) {
